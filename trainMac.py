...
def train_epoch(pretrain=False):
    ...
    for batch in trainloader:
        for sample in batch:
            vid = sample['vid']
            task = sample['task']
            if task not in Y or vid not in Y[task]:
                print(f"Missing video {vid} in task {task}. Skipping...")
                continue
            ...
            if pretrain:
                ...
                Y[task][vid].data = th.tensor(y, dtype=th.float).to(device)
            else:
                O = net(X, task)
    ...
    for batch in testloader:
        for sample in batch:
            ...
            if task not in Y_pred:
                Y_pred[task] = {}
            Y_pred[task][vid] = y
            annot_path = os.path.join(args.annotation_path, f"{task}_{vid}.csv")
...
if __name__ == "__main__":
    ...
    trainloader = DataLoader(trainset, batch_size=args.batch_size, num_workers=0, shuffle=True, drop_last=True, collate_fn=lambda batch: batch)
    ...
    net = Model(args.d, M, A, args.q).to(device)
    ...
    th.save(net.state_dict(), "final_model1.pth")
